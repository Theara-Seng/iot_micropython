...
def parse_cmd(text: str):
    """
    Returns (cmd, args) where cmd is lowercase without leading '/' and without '@bot' suffix.
    Examples:
      '/on' -> ('on','')
      '/on@aupp_relay_bot' -> ('on','')
      '/allow 123' -> ('allow','123')
      '/allow@aupp_relay_bot 123' -> ('allow','123')
    """
    if not text:
        return "", ""
    t = text.strip()
    if not t.startswith("/"):
        return "", t
    first = t.split()[0]              # e.g. '/on@bot'
    core = first[1:]                  # 'on@bot'
    core = core.split("@", 1)[0]      # 'on'
    cmd = core.lower()
    args = t[len(first):].strip()
    return cmd, args

def handle_cmd(chat_id, text):
    # Bootstrap admin (if none yet) and persist
    ensure_admin_and_allow(chat_id)

    # Gate: only allowed users/groups or admin
    ...
    if cmd in ("start", "help"):
        ...
        send_message(chat_id, "Your chat id: {}".format(chat_id))
    elif cmd == "status":
    ...
    elif cmd == "allow" and is_admin(chat_id):
        try:
            new_id = int(args)
            add_allowed(new_id)
    ...
    elif cmd == "revoke" and is_admin(chat_id):
        try:
            rid = int(args)
            remove_allowed(rid)
    ...
    elif cmd == "list" and is_admin(chat_id):
        send_message(chat_id, "Allowed: " + ", ".join(str(i) for i in sorted(ALLOWED)) +
                              ("\nAdmin: {}".format(ADMIN_CHAT_ID) if ADMIN_CHAT_ID is not None else ""))
    else:
